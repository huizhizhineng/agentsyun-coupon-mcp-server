...
## 测试
因为工具测试需要具体的经纬度，建议结合网络搜索、高德地图等可以获取到经纬度的工具进行测试，或者直接给出经纬度。

ps：具体调用该过程依赖于您选择的模型能力，如果没有成功获取到数据，可以通过调整提示词的方式引导模型识别。

由于本mcp 服务依赖于具体地址的经纬度，可以配合高德地图mcp的地址逆转工具或者联网搜索的功能获取具体地址的经纬度（输入的提示词中可以引导模型获取对应地址的经纬度），或者直接提具体的经纬度来调整提示词引导模型准确命中mcp具体的工具，如：
- "获取南京雨花台区汇智大厦附近的外卖优惠券短连接，搜索词为外卖"
- "获取南京雨花台区汇智大厦附近的外卖优惠券短连接，搜索词为咖啡"
- "获取南京雨花台区汇智城附近的外卖优惠券短连接5条，搜索词为咖啡，南京雨花台区汇智城的经纬度为：118.754352,31.974435"
